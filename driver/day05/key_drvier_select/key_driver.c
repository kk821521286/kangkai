...
void mukey_xiaodou (struct work_struct *work) 
{
	...
	if (mukey_val & (1 << 1)) {
		...
		//mukey DOWN 
		mukey_val = 'a' ;
		wake_up_interruptible (&read_header) ;
...
static ssize_t mukey_read (struct file* filp, char *buf, size_t len, loff_t *fpos)
{
	...
	if (mukey_val) {
		...
		wait_event_interruptible (read_header, mukey_val) ;
		if (0 == copy_to_user(buf, &mukey_val, len)) {
			mukey_val = 0 ;
			return len ;
		}
...
static unsigned int mukey_poll(struct file *filp, struct poll_table_struct *wait)                                    
{
	unsigned int mask = 0 ;
...
__init int mukey_init (void)
{
	...
	init_waitqueue_head (&read_header) ;
